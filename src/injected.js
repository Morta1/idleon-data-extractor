            ...
            for (let i in data) {
              ...
              try {
                ...
                if (
                  ...
                  const event = new CustomEvent("globalData", { detail: { name: 'save', data: jsonData } });
        ...
        this.addEventListener("message", function (event) {
          ...
          try {
            if (messageIntent.search(/_uid\//) !== -1) {
              ...
              const send = new CustomEvent("globalData", { detail: { name: 'charNames', data: jsonData } });
              window.dispatchEvent(send);
            } else if (messageIntent.search(/_guild\/[a-zA-Z0-9]*\/m$/) !== -1) {
              // data is guild member information
              const send = new CustomEvent("globalData", { detail: { name: 'guildInfo', data: jsonData } });
              window.dispatchEvent(send);
            }
          } catch (e) {
            // ignore
